...
export async function GET() {
  try {
    const kpis = await executeQuery<Record<string, number>>(`
      SELECT COUNT(*) AS TOTAL_ENTITIES, SUM(TOTAL_EVENTS) AS TOTAL_EVENTS, ROUND(AVG(SUCCESS_RATE), 1) AS AVG_SUCCESS_RATE, ROUND(SUM(TOTAL_VALUE)/1000000, 1) AS TOTAL_VALUE_M FROM CURATED.WELL_SUMMARY
    `);
